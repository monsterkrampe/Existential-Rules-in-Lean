...
@[grind ->]
theorem functional_term_for_var.inj {trg : PreTrigger sig} {i1 i2 : Nat} {v1 v2 : sig.V} :
  trg.functional_term_for_var i1 v1 = trg.functional_term_for_var i2 v2 -> i1 = i2 ∧ v1 = v2 := by unfold functional_term_for_var; simp
@[simp, grind =]
theorem functional_term_for_var.injEq {trg : PreTrigger sig} {i1 i2 : Nat} {v1 v2 : sig.V} :
  trg.functional_term_for_var i1 v1 = trg.functional_term_for_var i2 v2 ↔ i1 = i2 ∧ v1 = v2 := by unfold functional_term_for_var; simp
...
theorem apply_to_var_or_const_non_frontier_var (trg : PreTrigger sig) (i : Nat) :
    ∀ v, ¬ v ∈ trg.rule.frontier -> trg.apply_to_var_or_const i (.var v) = trg.functional_term_for_var i v := by
  intro v v_front
...
theorem apply_to_var_or_const_injective_of_not_in_frontier {v : sig.V}
    ...
    ∀ voc, (trg.apply_to_var_or_const i (VarOrConst.var v)) = (trg.apply_to_var_or_const i voc) -> VarOrConst.var v = voc := by
  intro voc apply_eq
  cases voc with
  | const c => simp [v_not_in_frontier, functional_term_for_var, GroundTerm.func_neq_const] at apply_eq
  | var u =>
    cases Decidable.em (u ∈ trg.rule.frontier) with
    | inl u_front =>
      apply False.elim
      rw [apply_to_var_or_const_non_frontier_var _ _ _ v_not_in_frontier, apply_to_var_or_const_frontier_var _ _ _ u_front] at apply_eq
...
theorem mem_fresh_terms {trg : PreTrigger sig} {i : Nat} {lt : i < trg.rule.head.length} :
  ...
  exists v; exact ⟨v_mem, Eq.symm t_mem⟩
...
theorem existential_var_for_fresh_term_after_functional_term_for_var
  ...
  . intro k k_lt
    ...
    rw [not_decide_eq_true, functional_term_for_var.injEq]
    simp only [true_and]
    exact k_lt
...
theorem atom_for_result_fact_mem_head {trg : PreTrigger sig} {f : Fact sig} {i : Fin trg.mapped_head.length}
    {f_mem : f ∈ trg.mapped_head[i.val]} : trg.atom_for_result_fact i f_mem ∈ trg.rule.head[i.val]'(by rw [← length_mapped_head]; exact i.isLt) := by
...
theorem var_or_const_for_result_term_mem_atom_for_result_fact {trg : PreTrigger sig} {f : Fact sig} {t : GroundTerm sig}
    {i : Fin trg.mapped_head.length} {f_mem : f ∈ trg.mapped_head[i.val]} {t_mem : t ∈ f.terms} :
    trg.var_or_const_for_result_term i f_mem t_mem ∈ (trg.atom_for_result_fact i f_mem).terms := by
...
theorem var_or_const_for_result_term_mem_terms_head {trg : PreTrigger sig} {f : Fact sig} {t : GroundTerm sig}
    {i : Fin trg.mapped_head.length} {f_mem : f ∈ trg.mapped_head[i.val]} {t_mem : t ∈ f.terms} :
    trg.var_or_const_for_result_term i f_mem t_mem ∈ (trg.rule.head[i.val]'(by rw [← length_mapped_head]; exact i.isLt)).terms := by
...
theorem apply_to_function_free_atom_eq_of_equiv {trg1 trg2 : PreTrigger sig} :
    trg1.equiv trg2 -> ∀ (i : Nat), ∀ a, trg1.apply_to_function_free_atom i a = trg2.apply_to_function_free_atom i a := by
  unfold equiv
  intro h i a
  apply TermMapping.apply_generalized_atom_congr_left
  intro voc voc_mem
