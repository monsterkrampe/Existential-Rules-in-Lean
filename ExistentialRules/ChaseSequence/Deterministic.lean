/-
Copyright 2026 Lukas Gerlach
Released under Apache 2.0 license as described in the file LICENSE.
-/

module

public import ExistentialRules.ChaseSequence.ChaseTree
import ExistentialRules.ChaseSequence.Universality

/-!
# The Chase on Deterministic Rules

In our most general setting, we consider existential rules with disjunctions.
Here, we consider the special case of deterministic rules, i.e. rules with exactly one head disjunct (so essentially no disjunction at all).
Here, `ChaseTree` and `ChaseBranch` consequently coincide. At least intuitively. Formally, we still need to implement the corresponding transitions.

Arguably the most interesting result of this file is `deterministicChaseBranchResultUniversallyModelsKb`.
It states that in the deterministic setting, the result of a `ChaseBranch` is itself a universal model.
That is, a model that can be homomorphically embedded into every other model [ChaseRevisited].
On disjunctive rules this is more complicated as can be seen in `chaseTreeResultIsUniversal`.
-/

public section

variable {sig : Signature} [DecidableEq sig.P] [DecidableEq sig.C] [DecidableEq sig.V]

namespace TreeDerivation

variable {obs : ObsolescenceCondition sig} {rules : RuleSet sig}

/-- The `firstBranch_from_start` of a `TreeDerivation` is generated by always picking the first child node by starting from a particular node in the tree. -/
def firstBranch_from_start {N : Type u} [CN : ChaseNode N obs rules]
    (td : TreeDerivation N obs rules) (start : NodeWithAddress td) : ChaseDerivation N obs rules :=
  let generator (node : NodeWithAddress td) : Option (NodeWithAddress td) := node.childNodes[0]?
  td.generate_subderivation start generator id
    (by intro node next next_mem
        simp only [generator] at next_mem
        simp only [id_eq]
        apply List.mem_of_getElem?; exact next_mem)
    (by intro node eq_none
        simp only [generator] at eq_none
        suffices node.subderivation.childNodes = [] by
          rw [childNodes_eq, List.map_eq_nil_iff] at this
          exact this
        rw [NodeWithAddress.childNodes_eq_childNodes, List.map_eq_nil_iff]
        rw [List.getElem?_eq_none_iff, Nat.le_zero, List.length_eq_zero_iff] at eq_none
        exact eq_none)

/-- In the deterministic setting, each derivation in a branches of a tree derivation node directly corresponds to the `firstBranch_from_start`. -/
theorem branches_start_eq_firstBranch_from_start {N : Type u} [CN : ChaseNode N obs rules]
    {td : TreeDerivation N obs rules} {start : NodeWithAddress td} (det : rules.isDeterministic) :
    ∀ deriv ∈ start.subderivation.branches, deriv = td.firstBranch_from_start start := by
  intro deriv deriv_mem
  rw [ChaseDerivation.mk.injEq, ChaseDerivationSkeleton.mk.injEq]
  apply PossiblyInfiniteList.ext
  intro n
  induction n generalizing deriv start with
  | zero =>
    rw [branches_eq] at deriv_mem
    rw [← PossiblyInfiniteList.head_eq, ← PossiblyInfiniteList.head_eq]
    simp only [firstBranch_from_start, generate_subderivation, derivation_for_branch]
    simp only [generate_branch, toFiniteDegreeTreeWithRoot]
    rw [FiniteDegreeTree.head_generate_branch, Option.map_some]
    simp only [Option.some_get]
    have head := deriv_mem.left
    unfold root ChaseDerivationSkeleton.head at head; rw [Option.get_inj] at head; exact head
  | succ n ih =>
    rw [branches_eq] at deriv_mem
    rw [← PossiblyInfiniteList.get?_tail, ← PossiblyInfiniteList.get?_tail]
    cases deriv_mem.right with
    | inl deriv_mem_r =>
      have contra : ¬∃ (trg : RTrigger obs rules), trg.val.active (CN.outgoingFacts deriv.head) := by
        rw [← deriv.isSome_next_iff_trg_ex, Option.isSome_iff_ne_none]; intro h; apply h; exact deriv_mem_r.right
      have head_eq_head : deriv.head = (td.firstBranch_from_start start).head := by
        rw [deriv_mem.left, NodeWithAddress.root_subderivation']
        simp only [firstBranch_from_start]
        rw [head_generate_subderivation]
        rfl
      have : (td.firstBranch_from_start start).next = none := by
        rw [head_eq_head] at contra
        apply Decidable.byContradiction
        intro contra'
        apply contra
        rw [← (td.firstBranch_from_start start).isSome_next_iff_trg_ex, Option.isSome_iff_ne_none]
        exact contra'
      unfold ChaseDerivationSkeleton.next at deriv_mem_r
      unfold ChaseDerivationSkeleton.next at this
      rw [← PossiblyInfiniteList.empty_iff_head_none] at deriv_mem_r
      rw [← PossiblyInfiniteList.empty_iff_head_none] at this
      rw [deriv_mem_r.right, this]
    | inr deriv_mem_r =>
      rcases deriv_mem_r with ⟨child, child_mem, next, next_eq, tail⟩
      have one_childTree : start.subderivation.childTrees.length = 1 := by
        rcases start.triggers_exist (by rw [childNodes_eq]; intro contra; rw [List.map_eq_nil_iff] at contra; apply List.ne_nil_of_mem child_mem; exact contra) with ⟨trg, _, facts_eq, _, _⟩
        have mapped_length_eq : start.subderivation.childNodes.length = (start.subderivation.childNodes.map CN.ingoingFacts).length := by simp
        rw [facts_eq, childNodes_eq, List.length_map, List.length_map] at mapped_length_eq
        rw [mapped_length_eq, PreTrigger.length_mapped_head]
        have := det trg.val.rule trg.property
        simp only [Rule.isDeterministic, decide_eq_true_eq] at this
        exact this
      have one_childNode : start.childNodes.length = 1 := by rw [NodeWithAddress.length_childNodes, childNodes_eq, List.length_map]; exact one_childTree
      have zero_lt_length : 0 < start.childNodes.length := by rw [one_childNode]; simp
      have first_child_isSome : start.childNodes[0]?.isSome := by rw [List.getElem?_eq_getElem zero_lt_length]; simp
      have : (deriv.tail (by rw [next_eq]; simp)) ∈ start.childNodes[0].subderivation.branches := by
        have : child = start.subderivation.childTrees[0]'(by simp [one_childTree]) := by
          rw [List.mem_iff_getElem] at child_mem
          rcases child_mem with ⟨i, h, child_mem⟩
          rw [one_childTree] at h
          simp only [Nat.lt_one_iff] at h
          simp only [h] at child_mem
          apply Eq.symm; exact child_mem
        have : child = start.childNodes[0].subderivation := by
          rw [this]
          conv => right; arg 0; unfold NodeWithAddress.subderivation
          simp only [NodeWithAddress.address_getElem_childNodes]
          simp only [NodeWithAddress.subderivation, childTrees, derivation_for_suffix]
          simp
        rw [← this]
        exact tail
      specialize ih _ this
      simp only [ChaseDerivation.tail, ChaseDerivation.derivation_for_skeleton, ChaseDerivationSkeleton.tail, ChaseDerivationSkeleton.derivation_for_branch_suffix] at ih
      simp only [firstBranch_from_start] at ih
      rw [ih, ← tail_generate_subderivation]; rfl
      rw [List.getElem?_eq_getElem zero_lt_length]
      simp

/-- The `firstBranch` of a `TreeDerivation` is generated by always picking the first child node starting from the root. -/
def firstBranch {N : Type u} [CN : ChaseNode N obs rules] (td : TreeDerivation N obs rules) : ChaseDerivation N obs rules :=
  td.firstBranch_from_start (NodeWithAddress.root td)

/-- The `firstResult` is the result of the `firstBranch`. -/
def firstResult (td : TreeDerivation (RegularChaseNode obs rules) obs rules) : FactSet sig := td.firstBranch.result

/-- The `firstResult` is a member of the `TreeDerivation.result`. -/
theorem firstResult_mem_result {td : TreeDerivation (RegularChaseNode obs rules) obs rules} : td.firstResult ∈ td.result := by
  unfold TreeDerivation.result; rw [Set.mem_map]
  exists td.firstBranch; constructor;
  . simp only [firstBranch, firstBranch_from_start]; exact td.generate_subderivation_mem_branches rfl
  . rfl

/-- In the deterministic setting, the branches of a `TreeDerivation` only contain the `firstBranch`. -/
theorem branches_eq_firstBranch_of_determinsitic {N : Type u} [CN : ChaseNode N obs rules] {td : TreeDerivation N obs rules} (det : rules.isDeterministic) :
    td.branches = fun b => b = td.firstBranch := by
  ext deriv
  constructor
  . intro deriv_mem; apply branches_start_eq_firstBranch_from_start det; rw [NodeWithAddress.subderivation_root]; exact deriv_mem
  . intro eq; rw [eq]; simp only [firstBranch, firstBranch_from_start]; exact td.generate_subderivation_mem_branches rfl

end TreeDerivation

namespace ChaseTree

variable {obs : ObsolescenceCondition sig} {kb : KnowledgeBase sig}

/-- In the deterministic setting, the `firstResult` of a `ChaseTree` is by itself a universal model. -/
theorem deterministicChaseTreeResultUniversallyModelsKb {ct : ChaseTree (RegularChaseNode obs kb.rules) obs kb} :
    kb.isDeterministic -> ct.firstResult.universallyModelsKb kb := by
  intro det
  constructor
  . apply ChaseTree.result_models_kb; exact ct.firstResult_mem_result
  . intro m m_is_model
    rcases chaseTreeResultIsUniversal ct m m_is_model with ⟨res, hom, res_mem, hom_is_hom⟩
    unfold TreeDerivation.result at res_mem; rw [Set.mem_map] at res_mem
    rcases res_mem with ⟨b, b_mem, res_mem⟩
    rw [TreeDerivation.branches_eq_firstBranch_of_determinsitic det] at b_mem
    unfold TreeDerivation.firstResult
    rw [← b_mem, res_mem]
    exact ⟨_, hom_is_hom⟩

end ChaseTree

namespace ChaseDerivation

variable {obs : ObsolescenceCondition sig} {rules : RuleSet sig}

/-- We can straightforwardly convert a `ChaseDerivation` into a `TreeDerivation` that has the original `ChaseDerivation` as its only branch. -/
def intoTree {N : Type u} [CN : ChaseNode N obs rules]
    (cd : ChaseDerivation N obs rules) (deterministic : rules.isDeterministic) : TreeDerivation N obs rules :=
  {
    tree := FiniteDegreeTree.from_branch cd.branch
    isSome_root := (by simpa using cd.isSome_head)
    triggers_exist := by
      intro ns node eq
      rw [FiniteDegreeTree.root_drop, Option.mem_def] at eq
      cases all_zero : ns.all (fun e => e = 0) with
      | false => rw [FiniteDegreeTree.get?_from_branch_of_some_ne_zero] at eq; simp at eq; grind
      | true =>
        rw [FiniteDegreeTree.get?_from_branch_of_all_zero (by grind)] at eq
        have trg_ex := cd.triggers_exist ns.length
        simp only [PossiblyInfiniteList.head_drop] at trg_ex
        specialize trg_ex _ eq
        cases after_eq : (cd.branch.drop ns.length).tail.head with
        | none =>
          intro contra; exfalso; apply contra
          rw [← List.head?_eq_none_iff, FiniteDegreeTree.drop_from_branch_of_all_zero (by grind), FiniteDegreeTree.head_childNodes_from_branch]
          exact after_eq
        | some after =>
          intro children_ne_nil
          specialize trg_ex after after_eq
          rcases trg_ex with ⟨orig_isSome, trg_ex⟩
          exists ((CN.origin after).get orig_isSome).fst
          constructor
          . let cd2 := cd.derivation_for_skeleton (cd.derivation_for_branch_suffix (cd.branch.drop ns.length) (PossiblyInfiniteList.IsSuffix_drop ns.length) (by simp [eq])) (PossiblyInfiniteList.IsSuffix_drop ns.length)
            have cd2_head_eq : cd2.head = node := by simp [cd2, ChaseDerivation.derivation_for_skeleton, ChaseDerivationSkeleton.derivation_for_branch_suffix, ChaseDerivationSkeleton.head, eq]
            -- rcases cd.subderivation_of_node_mem (by rw [cd.mem_iff]; exists ns.length) with ⟨cd2, cd2_head_eq, cd2_suf⟩
            suffices cd2.next = some after by rw [← cd2_head_eq]; exact cd2.active_trigger_origin_next this
            rw [← after_eq]
            simp [ChaseDerivationSkeleton.next, cd2, ChaseDerivation.derivation_for_skeleton, ChaseDerivationSkeleton.derivation_for_branch_suffix]
          suffices ((FiniteDegreeTree.from_branch cd.branch).drop ns).childNodes = [after] by
            have mapped_head_length : ((CN.origin after).get orig_isSome).fst.val.mapped_head.length = 1 := by
              rw [PreTrigger.length_mapped_head];
              have := deterministic _ ((CN.origin after).get orig_isSome).fst.property
              simp only [Rule.isDeterministic, decide_eq_true_eq] at this
              exact this
            have snd_zero : ((CN.origin after).get orig_isSome).snd.val = 0 := by
              have lt := ((CN.origin after).get orig_isSome).snd.isLt
              simp only [mapped_head_length, Nat.lt_one_iff] at lt
              exact lt
            simp only [this]; constructor
            . rw [List.map_singleton, trg_ex]
              rcases List.length_eq_one_iff.mp mapped_head_length with ⟨mapped_head, eq⟩
              rw [eq, List.map_singleton]
              rw [ChaseNode.origin_result_eq  orig_isSome (trg := ((CN.origin after).get orig_isSome).fst.val) (i := 0) rfl (Eq.symm snd_zero)]
              simp [eq]
            constructor
            . simp only [List.mem_singleton]; intro _ eq; rw [eq, Option.map_eq_some_iff]; exists (CN.origin after).get orig_isSome; simp
            . simp only [List.zipIdx_singleton, List.mem_singleton]; intro _ eq; rw [eq, Option.map_eq_some_iff]; exists (CN.origin after).get orig_isSome; simpa using snd_zero
          rw [← List.cons_head_tail children_ne_nil, List.cons_eq_cons]; constructor
          . apply List.head_of_head?_eq_some
            rw [FiniteDegreeTree.drop_from_branch_of_all_zero (by grind), FiniteDegreeTree.head_childNodes_from_branch]; exact after_eq
          . rw [← List.head?_eq_none_iff, List.head?_eq_getElem?, List.getElem?_tail]
            rw [FiniteDegreeTree.drop_from_branch_of_all_zero (by grind)]
            apply FiniteDegreeTree.get?_childNodes_from_branch_of_ne_zero
            simp
    fairness_leaves := by
      intro leaf
      rw [FiniteDegreeTree.mem_leaves]
      intro ⟨node, node_eq, node_children⟩ trg
      cases eq : node.all (fun e => e = 0) with
      | false => rw [FiniteDegreeTree.get?_from_branch_of_some_ne_zero] at node_eq; simp at node_eq; grind
      | true =>
        rw [FiniteDegreeTree.get?_from_branch_of_all_zero (by grind)] at node_eq
        rcases cd.fairness trg with ⟨fairness_step, fairness⟩
        have : fairness_step ≤ node.length := by
          rw [← List.head?_eq_none_iff, List.head?_eq_getElem?, FiniteDegreeTree.get?_childNodes, FiniteDegreeTree.get?_childTrees, FiniteDegreeTree.FiniteDegreeTreeWithRoot.opt_to_tree_after_tree_to_opt, FiniteDegreeTree.drop_drop, FiniteDegreeTree.root_drop] at node_children
          rw [FiniteDegreeTree.get?_from_branch_of_all_zero (by grind)] at node_children
          rw [List.length_append] at node_children
          apply Decidable.byContradiction
          intro lt; have le := Nat.succ_le_of_lt (Nat.lt_of_not_le lt)
          have := cd.branch.get?_eq_none_of_le_of_eq_none node_children _ le
          rw [PossiblyInfiniteList.head_drop] at fairness
          rw [this] at fairness
          simp at fairness
        cases Nat.lt_or_eq_of_le this with
        | inr eq =>
          have fairness := fairness.left
          rw [eq, PossiblyInfiniteList.head_drop] at fairness; simp only [node_eq] at fairness
          rcases fairness with ⟨_, eq, fairness⟩
          rw [Option.mem_def, Option.some_inj] at eq; rw [eq]
          exact fairness
        | inl lt =>
          have fairness := fairness.right (node.length - fairness_step - 1)
          rw [PossiblyInfiniteList.get?_tail, PossiblyInfiniteList.get?_drop] at fairness
          have : fairness_step + (node.length - fairness_step - 1).succ = node.length := by omega
          rw [this] at fairness
          simp only [node_eq] at fairness
          apply fairness
          simp
    fairness_infinite_branches := by
      intro trg
      rcases cd.fairness trg with ⟨fairness_step, fairness⟩
      exists fairness_step + 1
      intro ns ns_length
      cases eq : ns.all (fun e => e = 0) with
      | false => simp [FiniteDegreeTree.get?_from_branch_of_some_ne_zero (ns := ns) (by grind)]
      | true =>
        rw [FiniteDegreeTree.get?_from_branch_of_all_zero (by grind)]
        have fairness := fairness.right (ns.length - fairness_step - 1)
        rw [PossiblyInfiniteList.get?_tail, PossiblyInfiniteList.get?_drop] at fairness
        have : fairness_step + (ns.length - fairness_step - 1).succ = ns.length := by omega
        rw [this] at fairness
        exact fairness
  }

/-- The root of intoTree is the original head. -/
theorem root_intoTree {N : Type u} [CN : ChaseNode N obs rules] {cd : ChaseDerivation N obs rules} {det : rules.isDeterministic} :
    (cd.intoTree det).root = cd.head := by
  unfold intoTree TreeDerivation.root; simp [ChaseDerivationSkeleton.head]

/-- The first child node of intoTree is the original next node. -/
theorem next_intoTree {N : Type u} [CN : ChaseNode N obs rules] {cd : ChaseDerivation N obs rules} {det : rules.isDeterministic} :
    (cd.intoTree det).childNodes[0]? = cd.next := by
  unfold intoTree TreeDerivation.childNodes
  simp [ChaseDerivationSkeleton.next]

/-- In the context of any `TreeDerivation`, for a given start node and a `ChaseDerivation` cd, `firstBranch_from_start` for the start node returns exactly cd as long as the tree at the start node coincides the tree transformed cd. This is an auxiliary result for `firstBranch_intoTree_eq_self` below. -/
theorem firstBranch_from_start_with_intoTree_eq_cd {N : Type u} [CN : ChaseNode N obs rules]
    {td : TreeDerivation N obs rules} {cd : ChaseDerivation N obs rules} (det : rules.isDeterministic)
    {start : td.NodeWithAddress} (cd_eq : cd.intoTree det = start.subderivation) :
    td.firstBranch_from_start start = cd := by
  rw [ChaseDerivation.mk.injEq, ChaseDerivationSkeleton.mk.injEq]
  apply PossiblyInfiniteList.ext
  intro n
  induction n generalizing cd start with
  | zero =>
    have : (td.firstBranch_from_start start).head = cd.head := by
      simp only [TreeDerivation.firstBranch_from_start]; rw [TreeDerivation.head_generate_subderivation]
      rw [← TreeDerivation.NodeWithAddress.root_subderivation', id_eq, ← cd_eq]
      exact root_intoTree
    simp only [ChaseDerivationSkeleton.head, Option.get_inj, PossiblyInfiniteList.head_eq] at this
    exact this
  | succ n ih =>
    rw [← PossiblyInfiniteList.get?_tail, ← PossiblyInfiniteList.get?_tail]
    cases eq_next : cd.next with
    | none =>
      have : (td.firstBranch_from_start start).next = none := by
        simp only [TreeDerivation.firstBranch_from_start]; rw [TreeDerivation.next_generate_subderivation]; simp only [Option.map_eq_none_iff]
        rw [← next_intoTree (det := det), cd_eq, TreeDerivation.NodeWithAddress.childNodes_eq_childNodes, List.getElem?_map, Option.map_eq_none_iff] at eq_next
        exact eq_next
      unfold ChaseDerivationSkeleton.next at eq_next
      unfold ChaseDerivationSkeleton.next at this
      rw [← PossiblyInfiniteList.empty_iff_head_none] at eq_next
      rw [← PossiblyInfiniteList.empty_iff_head_none] at this
      rw [eq_next, this]
    | some next =>
      have next_isSome : cd.next.isSome := by rw [Option.isSome_iff_exists]; exists next
      have zero_lt_length : 0 < start.childNodes.length := by
        rw [← next_intoTree (det := det), List.getElem?_eq_some_iff] at eq_next
        rcases eq_next with ⟨lt, _⟩
        rw [TreeDerivation.NodeWithAddress.length_childNodes, ← cd_eq]
        exact lt
      let start' : td.NodeWithAddress := start.childNodes[0]
      have : (cd.tail next_isSome).intoTree det = start'.subderivation := by
        have : (cd.tail next_isSome).intoTree det = (cd.intoTree det).childTrees[0]'(by
            rw [TreeDerivation.NodeWithAddress.length_childNodes, TreeDerivation.childNodes_eq, List.length_map] at zero_lt_length
            rw [cd_eq]; exact zero_lt_length) := by
          simp only [tail, derivation_for_skeleton, ChaseDerivationSkeleton.tail, ChaseDerivationSkeleton.derivation_for_branch_suffix]
          simp only [TreeDerivation.childTrees, List.getElem_map, List.getElem_attach, TreeDerivation.derivation_for_suffix]
          rw [TreeDerivation.mk.injEq]
          simp only [intoTree]
          rw [FiniteDegreeTree.get_childTrees, FiniteDegreeTree.drop_from_branch_of_all_zero (by simp)]
          have aux : cd.branch.tail = (cd.branch.drop 0).tail := by simp
          rw [aux, PossiblyInfiniteList.tail_drop]
          rfl
        rw [this]; simp only [cd_eq]
        simp only [start', TreeDerivation.NodeWithAddress.subderivation, TreeDerivation.derivation_for_suffix, TreeDerivation.childTrees, List.getElem_map, List.getElem_attach]
        rw [TreeDerivation.mk.injEq, TreeDerivation.NodeWithAddress.address_getElem_childNodes, FiniteDegreeTree.get_childTrees, FiniteDegreeTree.drop_drop]
      specialize ih this
      simp only [TreeDerivation.firstBranch_from_start] at ih
      rw [← TreeDerivation.tail_generate_subderivation _ (by rw [List.getElem?_eq_getElem zero_lt_length]; simp [start'])] at ih
      exact ih

/-- The `firstBranch` of `intoTree` is the original `ChaseDerivation`. -/
theorem firstBranch_intoTree_eq_self {N : Type u} [CN : ChaseNode N obs rules] (cd : ChaseDerivation N obs rules) (det : rules.isDeterministic) :
    (cd.intoTree det).firstBranch = cd := by
  apply firstBranch_from_start_with_intoTree_eq_cd det
  rw [TreeDerivation.NodeWithAddress.subderivation_root]

/-- The `firstResult` of `intoTree` is the original `ChaseDerivationSkeleton.result`. -/
theorem firstResult_intoTree_eq_result (cd : ChaseDerivation (RegularChaseNode obs rules) obs rules) (det : rules.isDeterministic) :
    (cd.intoTree det).firstResult = cd.result := by
  unfold TreeDerivation.firstResult; rw [cd.firstBranch_intoTree_eq_self det]

end ChaseDerivation

namespace ChaseBranch

variable {obs : ObsolescenceCondition sig} {kb : KnowledgeBase sig}

/-- We can not only convert a `ChaseDerivation` into a `TreeDerivation` but also a `ChaseBranch` into a `ChaseTree`. -/
def intoTree {N : Type u} [CN : ChaseNode N obs kb.rules] (cb : ChaseBranch N obs kb) (det : kb.isDeterministic) : ChaseTree N obs kb :=
  let td := cb.toChaseDerivation.intoTree det
  {
    tree := td.tree
    isSome_root := td.isSome_root
    triggers_exist := td.triggers_exist
    fairness_leaves := td.fairness_leaves
    fairness_infinite_branches := td.fairness_infinite_branches
    database_first := by unfold td ChaseDerivation.intoTree; rw [FiniteDegreeTree.root_from_branch]; exact cb.database_first
  }

/-- In the deterministic setting, a `ChaseDerivationSkeleton.result` for a `ChaseBranch` is a universal model. -/
theorem deterministicChaseBranchResultUniversallyModelsKb (cb : ChaseBranch (RegularChaseNode obs kb.rules) obs kb) :
    kb.isDeterministic -> cb.result.universallyModelsKb kb := by
  intro det
  rw [← ChaseDerivation.firstResult_intoTree_eq_result cb.toChaseDerivation det]
  exact (cb.intoTree det).deterministicChaseTreeResultUniversallyModelsKb det

end ChaseBranch

