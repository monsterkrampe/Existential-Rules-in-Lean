...
theorem branches_start_eq_firstBranch_from_start
...
def firstResult (td : RegularTreeDerivation obs rules) : FactSet sig := RegularChaseDerivation.result td.firstBranch
...
def intoTree (cd : ChaseDerivation N obs rules) (deterministic : rules.isDeterministic) : TreeDerivation N obs rules :=
  {
...
theorem root_intoTree {cd : ChaseDerivation N obs rules} {det : rules.isDeterministic} :
...
theorem next_intoTree {cd : ChaseDerivation N obs rules} {det : rules.isDeterministic} :
...
theorem firstBranch_from_start_with_intoTree_eq_cd
...
theorem firstBranch_intoTree_eq_self (cd : ChaseDerivation N obs rules) (det : rules.isDeterministic) :
...
theorem firstResult_intoTree_eq_result (cd : RegularChaseDerivation obs rules) (det : rules.isDeterministic) :
    RegularTreeDerivation.firstResult (cd.intoTree det) = cd.result := by
  unfold RegularTreeDerivation.firstResult; rw [cd.firstBranch_intoTree_eq_self det]
...
theorem deterministicChaseBranchResultUniversallyModelsKb (cb : RegularChaseBranch obs kb) :
    kb.isDeterministic -> cb.result.universallyModelsKb kb := by
  intro det
  unfold RegularChaseBranch.result
  have eq := RegularChaseDerivation.firstResult_intoTree_eq_result cb.toChaseDerivation det
  unfold RegularChaseDerivation.result at eq
  rw [← eq]
  exact RegularChaseTree.deterministicChaseTreeResultUniversallyModelsKb (ct := cb.intoTree det) det
