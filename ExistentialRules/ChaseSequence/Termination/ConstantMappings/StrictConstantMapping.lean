...
/-- We lift `StrictConstantMapping`s to `FunctionFreeAtom`s in the obvious way. -/
abbrev apply_function_free_atom (g : StrictConstantMapping sig) : FunctionFreeAtom sig -> FunctionFreeAtom sig :=
  TermMapping.apply_generalized_atom g.apply_var_or_const

/-- Applying a `StrictConstantMapping` to a `FunctionFreeAtom` does not change the variables. -/
theorem apply_function_free_atom_vars_eq (g : StrictConstantMapping sig) (a : FunctionFreeAtom sig) :
    (g.apply_function_free_atom a).variables = a.variables := by
...
/-- We lift `StrictConstantMapping`s to `FunctionFreeConjunction`s in the obvious way. -/
