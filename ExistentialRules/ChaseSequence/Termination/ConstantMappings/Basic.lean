...
abbrev id : ConstantMapping sig := GroundTerm.const
...
theorem apply_pre_ground_term_congr_left (g g2 : ConstantMapping sig) (t : PreGroundTerm sig) :
    (∀ c ∈ t.leaves, g c = g2 c) -> g.apply_pre_ground_term t = g2.apply_pre_ground_term t := by
  intro h
...
theorem apply_pre_ground_term_eq_self_of_id_on_constants
    (g : ConstantMapping sig) (t : PreGroundTerm sig) (id_on_constants : ∀ c ∈ t.leaves, g c = .const c) :
    g.apply_pre_ground_term t = t := by
  unfold ConstantMapping.apply_pre_ground_term
...
theorem apply_ground_term_congr_left (g g2 : ConstantMapping sig) (t : GroundTerm sig) :
    (∀ c ∈ t.constants, g c = g2 c) -> g.apply_ground_term t = g2.apply_ground_term t := by
  intro h
...
theorem apply_ground_term_eq_self_of_id_on_constants
    (g : ConstantMapping sig) (t : GroundTerm sig) (id_on_constants : ∀ c ∈ t.constants, g c = .const c) :
    g.apply_ground_term t = t := by
  unfold ConstantMapping.apply_ground_term
  rw [Subtype.mk.injEq]
...
theorem apply_fact_eq_groundTermMapping_applyFact (g : ConstantMapping sig) (f : Fact sig) :
  g.apply_fact f = GroundTermMapping.applyFact g.apply_ground_term f := by rfl
