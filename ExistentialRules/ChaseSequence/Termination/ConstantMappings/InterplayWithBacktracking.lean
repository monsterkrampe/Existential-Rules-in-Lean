...
section InterplayWithBacktracking
  ...
  theorem GroundTerm.backtrackFacts_under_constant_mapping_subset_of_composing_with_subs
      ...
      ∀ (g : StrictConstantMapping sig), (∀ d, (d ∈ rl.rules.flatMap Rule.constants ∨ d ∉ term.constants) -> g d = d) ->
        ...
        (g.toConstantMapping.apply_fact_set backtracking.fst.toSet ⊆
        (backtrackFacts rl ((StrictConstantMapping.toConstantMapping (g2 ∘ g)).apply_ground_term term)
  ...
  theorem ChaseBranch.backtracking_of_loaded_trigger_in_node [GetFreshRepresentant sig.C] [Inhabited sig.C] (cb : ChaseBranch obs kb) (i : Nat) (node : ChaseNode obs kb.rules) (eq : cb.branch.infinite_list i = some node) :
      ...
      ∃ (g : ConstantMapping sig),
        g.apply_fact_set (trg.backtrackFacts rl (cb.trigger_ruleIds_valid_of_loaded i node eq rl rl_rs_eq trg trg_loaded) (cb.trigger_disjIdx_valid_of_loaded i node eq rl rl_rs_eq trg trg_loaded) (cb.trigger_rule_arity_valid_of_loaded i node eq rl rl_rs_eq trg trg_loaded)).toSet ⊆ node.fact.val ∧
