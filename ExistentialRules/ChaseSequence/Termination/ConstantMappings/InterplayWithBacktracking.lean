...
section InterplayWithBacktracking
  ...
  -- TODO: not sure if this one or the one below will work...
  ...
  /-       exists id -/
  /-       constructor -/
  /-       . simp only [GroundTerm.backtrackFacts, PreGroundTerm.backtrackFacts, GroundTerm.const] -/
  /-         intro f f_mem -/
  /-         rcases f_mem with ⟨f', f'_mem, f_eq⟩ -/
  /-         rw [List.mem_toSet] at f'_mem -/
  /-         simp at f'_mem -/
  ...
  /-       -- TODO: somehow get trigger that yields func term here... -/
  /-       have term_ruleIds_valid := cb.term_ruleIds_valid (i+1) node eq rl rl_rs_eq _ term_mem -/
  ...
  /-       let fresh_consts_for_pure_body_vars := GetFreshRepresentant.fresh_n forbidden_constants pure_body_vars.length -/
  /-       sorry -/
  ...
  theorem ChaseBranch.backtracking_of_term_in_node' [GetFreshRepresentant sig.C] [Inhabited sig.C] (cb : ChaseBranch obs kb) (i : Nat) (node : ChaseNode obs kb.rules) (eq : cb.branch.infinite_list i = some node) :
      ...
      ∀ (forbidden_constants : List sig.C), term.constants ⊆ forbidden_constants ->
      ∃ (g : ConstantMapping sig),
        g.apply_fact_set (term.backtrackFacts rl (cb.term_ruleIds_valid i node eq rl rl_rs_eq term term_mem) (cb.term_disjIdx_valid i node eq rl rl_rs_eq term term_mem) (cb.term_rule_arity_valid i node eq rl rl_rs_eq term term_mem) forbidden_constants).fst.toSet ⊆ node.fact ∧
        (∀ (d : sig.C), d ∈ forbidden_constants -> g d = .const d) := by
    intro rl rl_rs_eq term term_mem forbidden_constants forbidden_constants_subsumes
    ...
    | succ i ih =>
      ...
      cases f_mem with
      | inl f_mem =>
        rcases ih (cb.prev_node i (by simp [eq])) (by apply cb.prev_node_eq) term (by exists f) forbidden_constants forbidden_constants_subsumes with ⟨g, g_h⟩
        exists g
        constructor
      ...
      | inr f_mem =>
        ...
        | var v =>
            ...
            rcases ih (cb.prev_node i (by simp [eq])) (by apply cb.prev_node_eq) term (by
              ...
              . simp only [GroundSubstitution.apply_function_free_atom]
                ...
                . rw [voc_eq]
                  ...
                  . exact v_frontier
            ) forbidden_constants forbidden_constants_subsumes with ⟨g, g_h⟩
            exists g
            constructor
            . rw [cb.origin_trg_result_yields_next_node_fact i node eq]
              apply Set.subset_union_of_subset_left
          ...
          | inr v_frontier =>
            ...
            have : ∀ sublist, (sub : sublist ⊆ mapped_frontier) -> ∀ forbidden_constants, (sublist.flatMap GroundTerm.constants ⊆ forbidden_constants) -> ∃ (g : ConstantMapping sig),
                ...
                (∀ (d : sig.C), d ∈ forbidden_constants → g d = .const d) := by
              intro sublist sub forbidden_constants forbidden_constants_subsumes
              ...
              | nil =>
                ...
                . intros; rfl
              | cons hd tl inner_ih =>
                rw [List.flatMap_cons, List.append_subset] at forbidden_constants_subsumes
                ...
                rcases ih (cb.prev_node i (by simp [eq])) (by apply cb.prev_node_eq) hd (by
                  ...
                  . simp only [GroundSubstitution.apply_function_free_atom]
                    rw [List.mem_map]
                    exists .var v
                ) forbidden_constants forbidden_constants_subsumes.left with ⟨g_hd, g_hd_h⟩
                let t_res := PreGroundTerm.backtrackFacts rl hd.val hd.property
                  ...
                  (by have := func_term_rule_arity_valid.right
                      ...
                      exact (List.cons_subset.mp sub).left)
                  forbidden_constants
                rcases inner_ih (by apply List.subset_of_cons_subset; exact sub) (forbidden_constants ++ t_res.snd) (by apply List.subset_append_of_subset_left; exact forbidden_constants_subsumes.right) with ⟨g_tl, g_tl_h⟩
                    ...
                    have : g.apply_fact f' = g_hd.apply_fact f' := by
                      ...
                      | inr d_mem' =>
                        have : d ∈ forbidden_constants := by
                          have := PreGroundTerm.backtrackFacts_constants_in_term_or_fresh f' f'_mem d d_mem
                          cases this with
                          | inr d_mem'' => apply False.elim; apply d_mem'; exact d_mem''
                          | inl d_mem'' =>
                            apply forbidden_constants_subsumes.left
                            exact d_mem''
                  ...
                  | inr f'_mem =>
                    have : g.apply_fact f' = g_tl.apply_fact f' := by
                      ...
                      | inl d_mem' =>
                        ...
                        have := PreGroundTerm.backtrackFacts_list_constants_in_term_or_fresh f' f'_mem d d_mem
                        cases this with
                        | inl d_mem'' =>
                          apply d_not_forbidden
                          apply forbidden_constants_subsumes.right
            ...
            /- let rule := rl.get_by_id origin.fst.val.rule.id sorry -/
            /- have rule_eq : rule = origin.fst.val.rule := by sorry -/
            /- let pure_body_vars := rule.body.vars.filter (fun x => x ∉ rule.frontier) -/
            /- have pure_body_vars_eq : pure_body_vars = origin.fst.val.rule.body.vars.filter (fun x => x ∉ origin.fst.val.rule.frontier) := by sorry -/
            /- let fresh_consts_for_pure_body_vars := GetFreshRepresentant.fresh_n forbidden_constants pure_body_vars.length -/
            /- let backtrack_subs : GroundSubstitution sig := fun x => -/
            /-   if mem : x ∈ rule.frontier -/
            /-   then -/
            /-     let idx := rule.frontier.idxOf x -/
            /-     have : idx < (FiniteTreeList.fromList mapped_frontier.unattach).toList.length := by -/
            /-       sorry -/
            ...
            /-       /- exact mem -/ -/
            /-     ⟨(FiniteTreeList.fromList mapped_frontier.unattach).toList[idx], by -/
            /-       sorry -/
            ...
            /- let backtrack_trigger : PreTrigger sig := { rule, subs := backtrack_subs } -/
            ...
            /- have mapped_head_eq : backtrack_trigger.mapped_head[origin.snd.val]'(by rw [PreTrigger.result_eq_of_equiv _ _ triggers_equiv]; exact origin.snd.isLt) = origin.fst.val.mapped_head[origin.snd.val] := by -/
            /-   simp only [PreTrigger.result_eq_of_equiv _ _ triggers_equiv] -/
            ...
            rcases this mapped_frontier (by simp) (forbidden_constants ++ fresh_consts_for_pure_body_vars)
              (by apply List.subset_append_of_subset_left
                  intro d d_mem
                  apply forbidden_constants_subsumes
            ...
            have triggers_strong_equiv : origin.fst.val.strong_equiv { rule := backtrack_trigger.rule, subs := g.apply_ground_term ∘ backtrack_trigger.subs } := by
              ...
              . intro v v_mem
                ...
                | inr v_mem' =>
                  ...
                  simp only [pure_body_vars, rule] at this
                  ...
                  unfold g
                  have : fresh_consts_for_pure_body_vars.val[pure_body_vars.idxOf v]'(by unfold pure_body_vars; rw [fresh_consts_for_pure_body_vars.property.left]; apply List.idxOf_lt_length_of_mem; exact this) ∈ fresh_consts_for_pure_body_vars.val := by apply List.getElem_mem
              ...
              | inr f_mem =>
                ...
                have : g.apply_fact f = g_mapped_frontier.apply_fact f := by
                  ...
                  | inl d_mem' =>
                    ...
                    have := PreGroundTerm.backtrackFacts_list_constants_in_term_or_fresh f f_mem d d_mem
                    cases this with
                    | inl d_mem'' =>
                      apply d_not_forbidden
                      apply forbidden_constants_subsumes
