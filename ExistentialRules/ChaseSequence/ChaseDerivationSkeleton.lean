...
theorem mem_of_mem_suffix {cd1 cd2 : ChaseDerivationSkeleton obs rules} (suffix : cd1 <:+ cd2) : ∀ node ∈ cd1, node ∈ cd2 := by
  ...
  rcases suffix with ⟨m, suffix⟩
  exists m + n
  rw [← suffix, PossiblyInfiniteList.get?_drop] at node_eq
  exact node_eq
...
theorem subderivation_of_node_mem {cd : ChaseDerivationSkeleton obs rules} {node : ChaseNode obs rules} (node_mem : node ∈ cd) :
  ...
  constructor
  . simp [derivation_for_branch_suffix, head, node_mem]
...
theorem mem_iff_eq_head_or_mem_tail {cd : ChaseDerivationSkeleton obs rules} {node : ChaseNode obs rules} :
  ...
  . rw [mem_iff]
    intro ⟨n, mem⟩
    cases n with
    | zero => apply Or.inl; simp [head, PossiblyInfiniteList.head_eq, mem]
    | succ n =>
      apply Or.inr
      exists (by
        ...
        rw [cd.branch.get?_eq_none_of_le_of_eq_none contra n.succ (by simp)] at mem
        simp at mem)
      rw [mem_iff]
      exists n
      simpa [tail, derivation_for_branch_suffix] using mem
...
theorem suffix_iff_eq_or_suffix_tail {cd1 cd2 : ChaseDerivationSkeleton obs rules} : cd1 <:+ cd2 ↔ cd1 = cd2 ∨ ∃ h, cd1 <:+ cd2.tail h := by
  ...
  . rw [IsSuffix_iff]
    intro ⟨n, suffix⟩
    cases n with
    | zero => apply Or.inl; rw [PossiblyInfiniteList.drop_zero] at suffix; apply Eq.symm; rw [ChaseDerivationSkeleton.mk.injEq]; exact suffix
    | succ n =>
      apply Or.inr
      exists (by
        rw [Option.isSome_iff_ne_none]
        intro contra
        simp only [next, PossiblyInfiniteList.head_eq, PossiblyInfiniteList.get?_tail] at contra
        have := cd2.branch.get?_eq_none_of_le_of_eq_none contra (n+1) (by simp)
        rw [PossiblyInfiniteList.ext_iff] at suffix
        specialize suffix 0
        rw [PossiblyInfiniteList.get?_drop, Nat.add_zero, this] at suffix
        have contra := cd1.isSome_head
        rw [Option.isSome_iff_ne_none] at contra
        apply contra
        simp only [PossiblyInfiniteList.head_eq]
        rw [← suffix])
      rw [IsSuffix_iff]
      exists n
      simp only [tail, derivation_for_branch_suffix]
      grind
