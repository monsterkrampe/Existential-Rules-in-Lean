...
structure ChaseNode (obs : ObsolescenceCondition sig) (rules : RuleSet sig) where
  facts : FactSet sig
...
@[expose]
def origin_result {obs : ObsolescenceCondition sig} (node : ChaseNode obs rules) (isSome : node.origin.isSome) :
    List (Fact sig) :=
