...
/-!
# Cyclicity Derivations and Branches

This file formalizes what is called "cyclicity sequences" in the [RPC] paper (Section 3) including the main result
that the existence of a `CyclicityBranch` implies that the given rule set `RuleSet.neverTerminates` (see `CyclicityBranch.neverTerminates_of_cyclicityBranch`).
-/
