...
The definition captures both the Skolem and restricted chase at the same time through a generic `ObsolescenceCondition`.
Also, we are able to capture the core chase as well by allowing different implementations of `ChaseNode`s which provide ingoing and outgoing facts for each step in the chase.
For non-core chases (we call them regular), ingoing and outgoing facts are always the same. For the core chase, the outgoing facts are a core of the ingoing facts.
...
- A regular chase sequence without alternative matches on rules without disjunctions yields a universal model that is a core. (Section 3 of [this paper](https://iccl.inf.tu-dresden.de/web/Inproceedings3249))
- If a rule set is model-faithful acyclic (MFA), then every regular chase sequence on every database terminates. (The formalization is inspired by Section 3.1 in [this paper](https://iccl.inf.tu-dresden.de/web/Inproceedings3348); MFA was originally introduced [here](https://arxiv.org/abs/1406.4110).)
- A generalization of MFA that takes the obsolescence condition into account. This in principle generalizes both Disjunctive MFA (DMFA) and Restricted MFA (RMFA). Also our formalization adds support for constants in rules, which are not allowed in the original definitions. The only catch is that the original notions are specialized to handle only "Datalog-first" chase sequences. Since we consider arbitrary chase sequences here right now, the formalized versions are, strictly speaking, incomparable to the ones from the papers. In the future, we plan to fill this gap by formalizing Datalog-first chase sequences and the more specialized versions of DMFA and RMFA. (For DMFA see Section 3.2 in [this paper](https://iccl.inf.tu-dresden.de/web/Inproceedings3348). For RMFA see Section 4 of [this paper](https://iccl.inf.tu-dresden.de/web/Inproceedings3140). DMFA and RMFA are essentially the same except for the difference in obsolescence due to the different chase variants.)
- **WIP:** A generalized version of MFC/RPC-like non-termination conditions for the chase. We follow along the proof in [this paper](https://iccl.inf.tu-dresden.de/web/Inproceedings3353) with some small deviations. So far, we have formalized section 3 using `CyclicityDerivation`s.
